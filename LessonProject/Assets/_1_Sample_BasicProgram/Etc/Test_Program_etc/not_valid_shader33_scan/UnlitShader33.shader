    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float alpha = 1 - smoothstep(_LineSize, (_LineSize + _TrajectorySize) *_TrajectoryAlpha, linePosition);//同様にして徐々に透過させる
                
                float4 color = _LineColor * scanline + _TrajectoryColor * trajectory;//ここまでの計算結果を元に色を反映
                
                color.a = clamp(alpha *_AlphaFactor,0, _MaxAlpha);//透明度調整 clamp(a,b,c) aの値をb～cの間に収める
                return color;
            }
            ENDCG
        }
